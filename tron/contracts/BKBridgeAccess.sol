...
contract BKBridgeAccess is IBKBridgeAccess, IBKBridgeErrors, Ownable, Pausable, ReentrancyGuard {
    ...
    function setAccess(AccessType _accessType, bytes calldata _inputs) override external onlyOwner whenNotPaused {
    ...
    function checkBridgeReady() override external view returns (bool) {
        if (safe == address(0)) {
            revert NotSafe();
    ...
    function pause() override external onlyOperator {
        _pause();
    }

    function unpause() override external onlyOperator {
        _unpause();
    }

    function rescueERC20(address asset) override external onlyOperator {
        _checkZero(safe);
    ...
    function rescueETH() override external onlyOperator {
        _checkZero(safe);
